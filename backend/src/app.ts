...
app.get("/", (req, res) => {
  ...
  let htmlResponse = "<h3>🦄🌈✨ React Dashboard Admin V1 API ✨🌈🦄</h3>";
  htmlResponse += "<ul>";
...
app.get("/notification", (req, res) => {
  ...
  const search = (data: Notification[]) => {
  ...
  q
    ? res.json(search(notification).slice(0, 10))
    : res.json(notification.slice(0, 10));
});
...
app.get("/notes", (req, res) => {
  ...
  q ? res.json(search(notes).slice(0, 10)) : res.json(notes.slice(0, 10));
});
